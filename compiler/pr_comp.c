...
opcode_t pr_opcodes[] = {
    ...
    {"=", "STORE_FNC", 5, true, &def_function, &def_function,
     &def_function},
    ...
    {"=", "STOREP_FNC", 5, true, &def_pointer, &def_function,
     &def_function},
...
function_t *PR_ParseImmediateStatements(type_t * type)
{
    ...
    for (i = 0; i < type->num_parms; i++) {
        defs[i] =
            PR_GetDef(type->parm_types[i], pr_parm_names[i], pr_scope,
                      true);
    ...
    for (def = pr.def_head.next; def; def = def->next)
        if (!strcmp(def->name, name)) {
            ...
            if (type && def->type != type)
                PR_ParseError("Type mismatch on redeclaration of %s",
                              name);
            return def;
        }
