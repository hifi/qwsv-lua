...
void SV_Multicast(vec3_t origin, int to)
{
    ...
    case MULTICAST_PHS:
        mask =
            sv.phs + leafnum * 4 * ((sv.worldmodel->numleafs + 31) >> 5);
        break;
    ...
    case MULTICAST_PVS:
        mask =
            sv.pvs + leafnum * 4 * ((sv.worldmodel->numleafs + 31) >> 5);
        break;
...
// check for changes to be sent over the reliable streams to all clients
    for (i = 0, host_client = svs.clients; i < MAX_CLIENTS;
        ...
        if (host_client->old_frags != host_client->edict->v.frags) {
            for (j = 0, client = svs.clients; j < MAX_CLIENTS;
                 j++, client++) {
        ...
        if (val && host_client->entgravity != val->_float) {
            ...
            ClientReliableWrite_Float(host_client,
        ...
        if (c->num_backbuf) {
            ...
            if (c->netchan.message.cursize + c->backbuf_size[0] <
                ...
                if (c->num_backbuf) {
                    ...
                    c->backbuf.cursize =
                        c->backbuf_size[c->num_backbuf - 1];
                    c->backbuf.maxsize =
